...
A third subpath, **`./parse`**, exposes the same browser-faithful parse tree without any policy — for reading and querying HTML the way a browser builds it.
...
- **[Parsing (`neosanitize/parse`)](/parse)** — read and query the browser-faithful tree without sanitizing.
